...
async function conectarMetaMask() {
  ...
  try {
    provider = new ethers.providers.Web3Provider(window.ethereum); // ← V5 CORRETO
    await provider.send("eth_requestAccounts", []);
...
sendBtn.addEventListener("click", async () => {
  ...
  xhr.upload.addEventListener("progress", (e) => {
    if (e.lengthComputable) {
      ...
      result.innerHTML = `📤 Enviando... ${percent.toFixed(0)}%`;
    }
  });

  xhr.onloadstart = () => {
    result.innerHTML = "📡 Conectando ao servidor...";
  };
  ...
  xhr.onload = async () => {
    if (xhr.status === 200) {
      ...
      try {
        ...
        result.innerText = "⚠️ Erro ao registrar no contrato.";
      }

    } else {
      result.innerText = "❌ Erro no envio.";
    }
  };

  xhr.onerror = () => {
    result.innerText = "⚠️ Falha ao conectar ao servidor.";
  };
...
async function conectarMetaMask() {
  ...
  try {
    ...
    walletStatus.innerText = "✅ Conectado: " + conta.substring(0, 6) + "..." + conta.slice(-4);

    // Carrega ABI do contrato
    const response = await fetch("./Memoria.json");
    const contratoJson = await response.json();
    const signer = provider.getSigner();
    contrato = new ethers.Contract(contratoEndereco, contratoJson.abi, signer);
...
sendBtn.addEventListener("click", async () => {
  ...
  if (!conta) return alert("Conecte a MetaMask primeiro!");
  ...
  xhr.onload = async () => {
    if (xhr.status === 200) {
      try {
        ...
        alert("✅ Arquivo registrado na blockchain!");
        ...
        result.innerText = "⚠️ Erro ao registrar na blockchain.";
      }
    } else {
      result.innerText = "❌ Erro no envio.";
    }
...
sendCartaBtn.addEventListener("click", async () => {
  ...
  if (!texto) return alert("Digite sua carta primeiro!");
  if (!conta) return alert("Conecte a MetaMask primeiro!");

  const tituloFinal = diarioTitulo.value.trim() || "Carta digital";
  const dataFinal = document.getElementById("dataCarta").value || "";
  ...
  data: dataFinal
    ...
    if (response.ok) {
      ...
      alert("✅ Carta registrada com sucesso!");

      // Limpar campos
      cartaTexto.value = "";
      diarioTitulo.value = "";
      document.getElementById("dataCarta").value = "";
    } else {
      alert("❌ Erro ao enviar carta.");
    }
  } catch (err) {
